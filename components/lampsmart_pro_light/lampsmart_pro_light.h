...
class LampSmartProCommand {
 public:
  LampSmartProCommand(enum Variant variant, uint32_t id, uint8_t cmd)
      : variant_(variant), identifier_(id), cmd_(cmd) {}
...
class LampSmartProLight : public light::LightOutput,
                          public Component,
                          public Parented<LampSmartProQueue>
#ifdef USE_API
  ,
...
#endif
{
 ...
 protected:
  ...
  uint8_t min_brightness_{0x07};
  bool _is_off{true};
  uint8_t tx_count_{0};
  uint32_t tx_duration_{100};
  enum Variant variant_{VARIANT_3};
  light::LightState *light_state_{nullptr};
  LampSmartProQueue *queue_{nullptr};
};
...
template<typename... Ts> class PairAction : public Action<Ts...> {
 public:
  ...
  void play(Ts... x) override { ((LampSmartProLight *) this->state_->get_output())->on_pair(); }
...
template<typename... Ts> class UnpairAction : public Action<Ts...> {
 public:
  ...
  void play(Ts... x) override { ((LampSmartProLight *) this->state_->get_output())->on_unpair(); }
